...
import { DetailAkuntabilitas } from "../../../API/Home";
...
export default function DetailContentKecamatan() {
  ...
  const detailFileAkuntabilitas = useCallback(async (id: any) => {
    const data = await DetailAkuntabilitas(id);
    setFile(data);
  }, []);

  useEffect(() => {
    if (isReady) {
      detailFileAkuntabilitas(query.slug);
